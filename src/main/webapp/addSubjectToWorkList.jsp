...
$( document ).ready(function() {
   ...
   function getSubjects(projectID)
   {
		...
		$.ajax({         
			...
			success: function(response){
				...
				{
					var line = "<tr><td><span onclick=\"addToWorkList('" + projectID + "','" + subjects[i].subjectID +"')\"><u>" + subjects[i].subjectName + "</u></span></td><td>" + subjects[i].subjectID + "</td></tr>\n";
					html = html + line;
				}
				html = html + "</table>\n";
   ...
   function addToWorkList(projectID, subjectID)
   {
		var url = "<%=request.getContextPath()%>/v2/users/<%=wlusername%>/worklists/<%=workListID%>/projects/" + projectID + "/subjects/"  + subjectID + "?username=<%=username%>";
		$.ajax({         
			url: url,         
			type: 'put',         
			async: false,         
