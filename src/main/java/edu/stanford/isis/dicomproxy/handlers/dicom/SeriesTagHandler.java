...
public class SeriesTagHandler extends AbstractHandler {
     ...
     * NOTE: Apache/IO has a nice utility to do this, but we don't have it here, so doing this instead.
     ...
     * @throws IOException on error
     */
    private String base64Encode(ByteArrayOutputStream buffer)
        throws IOException
    {
        buffer.flush();
    ...
    private static String getStudyTags(String studyIUID)
    ...
    private static String getPatientTags(String patId)
        throws IOException, DicomException
    {
        StringBuilder sb = new StringBuilder();
    ...
    private static InputStream getInstanceTagsForSeriesAsStream(String seriesIUID)
        throws SQLException, DicomException, IOException
    {
        MySqlQueries queries = MySqlInstance.getInstance().getMysqlQueries();
    ...
    private static String getInstanceTagsForSeries(String seriesIUID)
        throws SQLException, DicomException, IOException
    {
        MySqlQueries queries = MySqlInstance.getInstance().getMysqlQueries();
        Blob blob = queries.getImageBlobDataForSeries(seriesIUID);
    ...
    private static String getInstanceTags(InputStream stream)
        throws IOException, DicomException
    {
        StringBuilder sb = new StringBuilder();
        ...
        //createDicomTagEntry(sb,attributeList,new AttributeTag(0x0020,0x1041));//Slice Location -->//different for each image.
...
//        createDicomTagEntry(sb,attributeList,new AttributeTag(0x0040,0xA375));//Current Requested Procedure Evidence Sequence -->
...
//        createDicomTagEntry(sb,attributeList,new AttributeTag(0x0400,0x0561));//Original Attributes Sequence -->
    ...
    private static String getSeriesTags(String seriesIUID)
        throws IOException, DicomException
    {
        StringBuilder sb = new StringBuilder();
