...
public class SeriesOrder {
    ...
    public SeriesOrder(int numInstance, String seriesUID){
        ...
        if(numInstance<1){throw new IllegalArgumentException("numInstances must be a positive value.");}
    ...
    public ImageEntry getByInstanceNo(int index)
        throws Exception
    {
        try{
...
//            //ToDo: delete above once debugged.
     ...
     */
    public void addNewImage(int instNum, String sopInstanceUID){
        ...
        if( !hasInstance(instNum) ){
            ...
            if(instances.size()<instNum+1)
            {
                ...
                logger.info("WARNING: resizing array from="+instances.size()+" to="+(instNum+1)+" series="+seriesUID);
