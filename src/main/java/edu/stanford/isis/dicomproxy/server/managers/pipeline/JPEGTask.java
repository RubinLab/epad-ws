...
public class JPEGTask implements Callable<File>
{
    ...
    public File call() throws Exception
    {
        ...
        try{
            ...
            closeReader(isr);
            if(is!=null){
                try{is.close();is=null;}catch(Exception e){log.warning("Failed to close InputStream.",e);}
            }
            if(process!=null){
                try{process.destroy();}catch(Exception e){log.warning("Failed to destroy process.",e);}
            }
        }
    ...
    /**
     * Write a JPEG that is created using the following parameters. The resulting
     * file for a file with the settings, window=4096 and center=2048 will have the
     * following name.
     *
    ...
    public void writeJpegsLeveled(int width, int level){
        ...
        try{
            ...
            //format currently is dcm2jpg dcmFileIn jpgFileOut
            String[] command = new String[]{"./dcm2jpg","-c="+level,"-w="+width,file.getAbsolutePath(),jpegFile.getAbsolutePath()};
    ...
    /**
     * Using the current dicom file read the tag to figure out if a thumbnail file should
     * be created using it.
