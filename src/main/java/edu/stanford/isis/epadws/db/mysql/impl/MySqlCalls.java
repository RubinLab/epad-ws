...
public class MySqlCalls {
    ...
    //public static final String SELECT_FILES_FOR_SERIES = "SELECT i.sop_iuid, i.inst_no, s.series_iuid, f.filepath, f.file_size from pacsdb.files as f, pacsdb.instance as i, pacsdb.series as s WHERE f.instance_fk=i.pk and i.series_fk=s.pk and s.series_iuid=?";
    ...
    //public static final String SELECT_SERIES_FOR_STUDY = "SELECT s.series_iuid, p.pat_id, p.pat_name, st.study_datetime, s.modality, s.series_desc, s.num_instances from pacsdb.series as s, pacsdb.study as st, pacsdb.patient as p where st.study_iuid=? and s.study_fk=st.pk and st.patient_fk=p.pk";
    ...
    public static final String INSERT_INTO_EPAD_FILES = "INSERT INTO epaddb.epad_files"
                        +"(instance_fk,file_type,file_path,file_size,file_status,err_msg,file_md5)"
                        +"VALUES (?,?,?,?,?,?,?)";
    
    public static final String INSERT_INTO_USER = "INSERT INTO epaddb.users"
        +"(user_name,user_email,user_passwd,user_expiration,user_role)"
        +"VALUES (?,?,?,?,?)";
