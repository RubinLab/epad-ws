...
/**
 * Watches for a new directory and a zip file. When a new directory is
 * found it puts a "dir.found" file into it and passes it to the
 * DicomSendTask.
 *
 * @author amsnyder
 */
public class MySqlUploadDirWatcher implements Runnable
{
    ...
    private boolean waitOnEmptyDir(File dir)
        throws InterruptedException
    {
    ...
    private File waitForZipUploadComplete(File dir)
        throws InterruptedException
    {
        ...
        while(true){
            ...
            if(zipFiles==null){
                ...
                throw new IllegalStateException("Too many zip files ("+numZipFiles+") in directory. dir="+dir.getAbsolutePath());
            }
    ...
    private void unzipFiles(File zipFile)
        throws IOException
    {
    ...
    private void sendDicom(File dir)
        throws Exception
    {
