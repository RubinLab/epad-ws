...
/**
 * This is a process the runs every five seconds. If it sees a new file
 * in two consecutive runs with the same file size it will assume the it is
 * "DONE" and put it into the QUEUE for the unzipping files. This begins
 * the process.
 */
public class UploadDirWatcher implements Runnable{

    public static final String UPLOAD_ROOT_DIR = "../resources/upload/";
    public static final int CHECK_INTERVAL = 5000; //check every 5 seconds.

    ProxyLogger log = ProxyLogger.getInstance();

    private BlockingQueue<File> unzipQueue;

    /**
     * Keeps track of files that are new, but not in the queue. If the file size and time-stamp does
     * not change for 5 seconds then assume it is ready for processing.
     */
    private final Map<FileKey,UploadFile> waitingForPipelineMap = new HashMap<FileKey, UploadFile>();
    ...
    /**
     * Track /resource/upload/temp-* directories that are empty (no *.dcm files) and delete them if
     * they remain empty for a certain period of time. (5 min).
     * The Long value is the timestamp of when it is first seen as empty.
     */
            ...
            if(currFile.canRead()){
                ...
                if( !isError && !isInPipeline ){
                    ...
                    if( shouldAddToPipeline ){
                        ...
                        boolean accepted = unzipQueue.offer(
                                            currFileKey.getFile(),100, TimeUnit.MILLISECONDS);
                        if(accepted){
    ...
    private boolean checkShouldAddToPipeline(FileKey fileKey){
        ...
        if( waitingForPipelineMap.containsKey(fileKey) ){
            ...
            UploadFile currFileState = new UploadFile(fileKey.getFile());
            UploadFile prevFileState = waitingForPipelineMap.get(fileKey);
            ...
            waitingForPipelineMap.put(fileKey,new UploadFile(fileKey.getFile()));
        }
        return false;
    }

    /**
     * Return true if this is an extension that is be added to the pipeline. Those are
     * zip, dcm, and gz.
