...
public class SeriesFileUtils {
     ...
     */
    public static Map<String,String> readSeriesFile(String seriesFilePath)
        throws IOException
    {
    ...
    public static void writeSeriesFile(String seriesFilePath, Map<String,String> seriesFileTags){
        ...
        if( seriesFileTags.get(SeriesFileUtils.IMAGES_IN_SERIES)==null){
    ...
    public static String createSeriesFileContent(Map<String,String> mapSeriesFileTags){
        ...
        sb.append(getEitherTagType(mapSeriesFileTags,DicomTagFileUtils.SERIES_DESCRIPTION,SERIES_DESC,"unknown")).append(",");
        sb.append(getEitherTagType(mapSeriesFileTags,NUM_SERIES_REL_INSTANCES,NUM_SERIES_REL_INSTANCES,"unknown")).append(",");
        sb.append(getEitherTagType(mapSeriesFileTags,IMAGES_IN_SERIES,IMAGES_IN_SERIES,"unknown"));

        return sb.toString();
    }

    private static String getEitherTagType(Map<String,String> tagMapUnknownType,
                                           String dicomTag, String seriesTag, String fallback)
    {
    ...
    /**
     * Parse all the *.series files in a directory. It reads the series files
     * into a generic map, so they can be written out. A Map is used so additions
     * or changes to the files with minimal changes in other places.
     *
     *
    ...
    public static List<File> getSeriesFileFor(String studyId){
        ...
        try{
            ...
            File studyDir = new File("../resources/dicom/"+studyDirName);
            if( !studyDir.exists() ){
                log.info("WARNING: Could not find directory for: "+studyId
                        +". Dir doesn't exist. dir="+studyDir.getCanonicalPath());
                return retVal;
            }
