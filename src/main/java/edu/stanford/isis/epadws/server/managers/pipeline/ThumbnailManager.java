 ...
 *
 * On start-up it will look at the the series, to see if any have missing thumbnail files.
 * If a thumbnail file is missing for a series it will read the order file to select which
 * dicom file is used to generate the thumbnail. This file is the put into a the
 * "generateThumbnail" queue.
 *
 * When a new series is uploaded, the mover stage of the pipeline will ask this class
 * if that file should be used to generate a thumbnail. This class will use the following
 * rules to respond.
 *
 * (a) If it is the first dcm file for that series then respond yes.
 *
 *  From that point it keeps track of the InstanceNumbers to select a dicom file near the
 *  middle of the stack. It will keep track of the min, and max InstanceNumbers along with the
 *  most recent used to generate a thumbnail. If the most recent falls outside of the middle 25%
 *  of the min/max range then it will look for a new file that is closer to the most recent one
...
public class ThumbnailManager {
    ...
    /**
     * Map of String to ThumbnailInfo
     */
...
//    private static byte[] createTwoChannelPngData(byte[] rawDicom, TransferSyntax transferSyntax, float slope, float intercept, int xSize, int ySize)
//    {
//
...
//    }
    ...
    static class ThumbnailInfo {
        ...
        /**
         * Compare the maxOrder, minOrder and lastOrder. If the lastOrder is outside
         * an acceptable range then return true, and set the internal values.
         *
    ...
    static enum ThumbnailStatus {
        NO_THUMBNAIL,
        PRE_EXISTING_THUMBNAIL,
        NEW_SERIES_HAS_GOOD_THUMBNAIL,
        NEW_SERIES_NEEDS_BETTER_THUMBNAIL;
    }
