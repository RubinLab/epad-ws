 ...
 */
public class OrderFileRunnable implements Runnable{
    ...
    public OrderFileRunnable(ExecutorService exec, BlockingQueue<File> orderFileQueue){
        this.exec = exec;
    ...
    public void run() {
        ...
        try{
            ...
            do{
                ...
                //log how long process took to characterize speed.
                if(allOrderFilePath.size()>0){
                    logger.info("Updating "+allOrderFilePath.size()
                            +" series took: "+(System.currentTimeMillis()-startTime)+"ms. ");
                }

                //check for shutdown before waiting for one second.
                if(signal.hasShutdown()){
                    break;
                }
    ...
    private static void updateSeriesFileWithImageCount(String orderFilePath){
        ...
        try{
            ...
            Map<String,String> dataMap = SeriesFileUtils.readSeriesFile(seriesFilePath);
            dataMap.put(SeriesFileUtils.IMAGES_IN_SERIES,""+nDcmFiles);
            logger.info("Updating _"+seriesFilePath+"_ with IMAGES_IN_SERIES="+nDcmFiles);
            SeriesFileUtils.writeSeriesFile(seriesFilePath,dataMap);
     ...
     *
     * from:
     *    ./resources/dicom/1_2_3_456/series_1_4_789.order
     * to:
     *    ./resources/dicom/1_2_3_456/1_4_789
     *
     ...
     */
    private static void updateOrderFileWith(String orderFilePath, List<OrderFileEntry> entriesToAdd)
        throws IOException
    {
        ...
        List<ImageOrder> imageOrderList = OrderFileUtils.readOrderFileFromTags(orderFile);
        List<ImageOrder> updatedOrderList = insertNewImages(imageOrderList, entriesToAdd);
        try{
            OrderFileUtils.writeOrderFile(updatedOrderList, orderFile);
     ...
     */
    private static List<ImageOrder> insertNewImages(List<ImageOrder> fileList, List<OrderFileEntry> entriesToAdd){
        Set<ImageOrder> fileSet = new HashSet<ImageOrder>(fileList);
        for(OrderFileEntry currEntry : entriesToAdd){
            ...
            fileSet.add( new ImageOrder(order, sopInstanceID, currEntry.tagsMap) );
        }

        List<ImageOrder> retVal = new ArrayList<ImageOrder>(fileSet);
        retVal = OrderFileUtils.removeDuplicateInstanceNumbers(retVal);
    ...
    /**
     * Reads all new the *.tag files in the queue to determine which series they belong to, and
...
//            logger.debug("organizeBySeriesName: "+currFile.getAbsolutePath());
            ...
            try {
                ...
                File currOrderFile = OrderFileUtils.createOrderFile(currStudyUID, currSeriesUID);
                ...
                if( entryList==null ){
                    ...
                    entryList = OrderFileUtils.readOrderFile(key,currTagsMap, currFile.getName());
