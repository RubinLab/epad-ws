...
public interface Dcm4CheeDatabaseCommands
{
	...
	public static final String SELECT_INSTANCE_FOR_SERIES = "SELECT sop_iuid from pacsdb.instance as i, pacsdb.series as s where i.series_fk=s.pk and s.series_iuid=? order by i.inst_no";
	...
	public static final String SELECT_SERIES_BY_ID = "SELECT st.study_iuid, s.series_iuid, p.pat_id, p.pat_name, st.study_datetime, s.modality, s.series_desc, s.num_instances, s.series_status, s.body_part, s.institution, s.station_name, s.department from pacsdb.series as s, pacsdb.study as st, pacsdb.patient as p where s.series_iuid=? and s.study_fk=st.pk and st.patient_fk=p.pk";
