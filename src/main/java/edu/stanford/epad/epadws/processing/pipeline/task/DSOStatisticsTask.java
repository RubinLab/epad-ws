 ...
 * 
 */
public class DSOStatisticsTask implements Runnable
{
	...
	public DSOStatisticsTask(String username, String projectID, String referencedSeriesUID, String[] referencedImageUID, String aimUID, String dsoFileName)
	{
		this.username = username;
		this.projectID = projectID;
		this.referencedSeriesUID = referencedSeriesUID;
	...
	public void run()
	{
		try {
			...
			List<ImageAnnotationCollection> iacs = AIMQueries.getAIMImageAnnotationsV4(projectID, AIMSearchType.ANNOTATION_UID, aimUID, username);			
			if (iacs.size()==1){
				aim = new Aim4(iacs.get(0));
				dc = (DicomSegmentationEntity) aim.getSegmentationEntityCollection().getSegmentationEntityList().get(0);
			}else {
				log.warning("Couldn't find a specific aim to calculate statistics on for aim uid "+ this.aimUID);
				return;
			}
			...
			log.warning("Error calculate statistics for DSO:" + dsoFileName, e);
		}
	}
}
